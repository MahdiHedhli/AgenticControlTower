...
def generate_act_toml(
    *,
    agent_id: str = "act-local-agent",
    gated_tools: tuple[str, ...] | None = None,
) -> str:
    """Render ``act.toml`` — the bridge config consumed in Phase 3. Points at
    the loopback gateway and lists the clearance-gated tools."""
    ...
    gated = ", ".join(_toml_str(t) for t in gated_tools)
    return "\n".join(
        [
